...
```{r results='hide'}
list.files('../data') #Getting the data with the files
```

```{r results='hide', message=FALSE}
half_hourly_fluxes <- readr::read_csv("../data/df_for_stepwise_regression.csv") # data transfering into R
```

```{r}
visdat::vis_miss(half_hourly_fluxes) #looking if there are any missing values
```


It can be seen that some rows contain missing values. It will be important to keep that in mind for the interpretation of the results and for the creation of the model. Rows with missing data will be deleted to work with the data.


```{r results='hide'}
summary(half_hourly_fluxes) #looking where the missing data in which row is
```

```{r warning=FALSE, message=FALSE}
half_hourly_fluxes_new_order <- dplyr::select(half_hourly_fluxes,`GPP_NT_VUT_REF`, everything()) #moving the column of the response variable (GPP) to the first position
```
...
# Going through all variables to build all the bivariate models
...
# Convertin AIC to the numeric format
...
# Print the results of the ten best AIC values
print(results_bivariate[1:10,])
```


Here we have a collection of bivariate models sorted in ascending order by the AIC. This means that the topmost model is considered the best since it has the lowest AIC value. In this particular case, the predictor "PPFD_IN" emerges as the best option. Since "<none>" is not the optimal choice, it is logical to continue improving the model through a stepwise forward regression.

Moreover, it is noticeable that both the R-squared and adjusted R-squared values increase as we move higher up the list (indicating better AIC values). This observation fits with the idea that a higher R-squared indicates a better model of the data. Similarly, a higher adjusted R-squared suggests increased generalizability of the model. Consequently, it is reasonable for the best model (with the highest AIC) to also possess the highest adjusted R-squared. While it is possible for the best model to not have the highest R-squared, in this case, the highest R-squared value corresponds to the lowest AIC. This relationship makes sense since the R-squared metric evaluates the model's performance on the existing data, whereas the AIC considers the model's suitability for new data.

### Plotting the best value
```{r warning=FALSE, message=FALSE}
library(ggplot2)
...
There appears to be a potential linear correlation between these two values, but it is not possible to make a statements about their dependence since any observed correlation could be a coincidence. However, it is evident that the GPP_NT_VUT_REF values cannot be accurately described solely by a linear regression model with PPFD_IN as the predictor. Consequently, it is reasonable to proceed with a stepwise approach in an attempt to enhance the model and capture the underlying relationships more effectively.
...
# Naming
response_var <- "GPP_NT_VUT_REF"
# Vector to store the selected predictors
...
The selection of the model is justified by opting for the lowest AIC, which signifies the best model. Additionally, it is reasonable that the process stopped after including the predictor P_F, as further additions did not improve the AIC values. It is worth noting that the highest R-squared value is not necessarily found within the best model. This discrepancy arises because R-squared only represents the performance of the model on the specific data used for fitting. On the other hand, adjusted R-squared incorporates the model's generalizability and its suitability for new data with the same response variable and predictors.The step function identifies the model with the lowest AIC, thereby determining the best model. Therefore, the AIC value plays a decisive role in the selection process.
...
### Other option to handle the missing data - Imputing the data
...
```{r eval=FALSE}
plot(imp_data) #checking if convergence was correctly achieved
```
